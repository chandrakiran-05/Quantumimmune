...
export default function ComparePage() {
  ...
  if (error || !data) return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      className="border border-accent/20 bg-accent-tint/10 p-10 rounded-2xl text-center space-y-4 max-w-md mx-auto">
      ...
      <button onClick={() => window.location.reload()} className="px-5 py-2 border border-border text-xs font-bold bg-white rounded-xl hover:bg-card transition-colors cursor-pointer">Retry</button>
    </motion.div>
  );
  ...
  return (
    <div className="space-y-20">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 200, damping: 25 }}
        ...
        <h1 className="text-4xl font-bold tracking-tight text-foreground font-display leading-tight">Performance metrics & concurrency speedups.</h1>
        <p className="text-secondary text-sm">Quantum Kernel SVM (multi-class, one-vs-rest) versus classical baselines.</p>
      </motion.div>

      {/* Metrics Table */}
      <Reveal>
        <h2 className="text-[11px] uppercase tracking-[0.15em] font-bold text-accent font-data mb-5">Accuracy & Separation</h2>
            ...
            <thead>
              <tr className="border-b border-border bg-card">
                {["Model", "Accuracy", "F1 Macro", "Precision", "Recall", "Train Time"].map(h => (
                  <th key={h} className={`px-5 py-3.5 text-[10px] font-bold text-secondary uppercase tracking-wider font-data ${h !== "Model" ? "text-right" : ""}`}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {models.map((name, idx) => {
                ...
                return (
                  <motion.tr key={name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    className="group hover:bg-accent-tint/10 transition-colors duration-200">
                    <td className="px-5 py-3.5 text-sm font-bold text-foreground group-hover:text-accent transition-colors duration-200">{name}</td>
                    <td className="px-5 py-3.5 text-sm font-data text-right text-accent font-bold"><CountUp target={m.accuracy * 100} suffix="%" /></td>
                    <td className="px-5 py-3.5 text-sm font-data text-right">{m.f1_macro.toFixed(4)}</td>
                    <td className="px-5 py-3.5 text-sm font-data text-right">{m.precision_macro.toFixed(4)}</td>
                    <td className="px-5 py-3.5 text-sm font-data text-right">{m.recall_macro.toFixed(4)}</td>
                    <td className="px-5 py-3.5 text-sm font-data text-right text-secondary">{m.train_time.toFixed(3)}s</td>
                  </motion.tr>
                );
              })}
            </tbody>
      ...
      <Reveal>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
          <h2 className="text-[11px] uppercase tracking-[0.15em] font-bold text-accent font-data">Confusion Matrices</h2>
          <div className="flex bg-card border border-border p-1 rounded-xl">
            {models.map(name => (
              <button key={name} onClick={() => setActiveTab(name)}
                className={`relative px-4 py-2 text-[11px] font-bold rounded-lg cursor-pointer transition-colors duration-200 ${activeTab === name ? "text-accent" : "text-secondary hover:text-foreground"}`}>
                {activeTab === name && <motion.span layoutId="cm-tab" className="absolute inset-0 bg-white border border-border rounded-lg shadow-sm -z-10" transition={{ type: "spring", stiffness: 400, damping: 30 }} />}
                {name}
              </button>
            ))}
          </div>
      ...
      {/* Speedup */}
      <Reveal>
        <h2 className="text-[11px] uppercase tracking-[0.15em] font-bold text-accent font-data mb-5">Parallel Speedup (Classical CPU)</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
          {[
            { label: "Sequential (1 Thread)", value: data.benchmarks.sequential_time, suffix: "s", decimals: 3, sub: `${data.benchmarks.sequential_pairs} pairs`, highlight: false },
            { label: `Parallel (${data.benchmarks.n_cores} Threads)`, value: data.benchmarks.parallel_time, suffix: "s", decimals: 3, sub: "multiprocessing", highlight: false },
            { label: "Speedup Factor", value: data.benchmarks.speedup, suffix: "×", decimals: 2, sub: "concurrency multiplier", highlight: true },
          ].map((card, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ delay: idx * 0.1, type: "spring", stiffness: 300, damping: 25 }}
              whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(43,36,32,0.1)" }}
              className={`border p-6 rounded-2xl cursor-default ${card.highlight ? "border-accent/30 bg-accent-tint/10 border-t-4 border-t-accent" : "border-border bg-white"}`}>
              <span className={`text-[10px] font-data uppercase tracking-wider ${card.highlight ? "text-accent" : "text-secondary"}`}>{card.label}</span>
              <div className={`mt-2 text-3xl font-bold font-data ${card.highlight ? "text-accent" : "text-foreground"}`}>
                <CountUp target={card.value} decimals={card.decimals} suffix={card.suffix} />
              </div>
              <span className="text-[9px] text-secondary mt-1 font-data block">{card.sub}</span>
            </motion.div>
          ))}
        </div>
          ...
          <div className="space-y-4 max-w-xl mx-auto">
              ...
              <div key={idx} className="flex items-center gap-4">
                ...
                <div className="flex-1 h-4 bg-border/20 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} whileInView={{ width: `${bar.ratio * 100}%` }} viewport={{ once: true }}
                    transition={{ delay: 0.2 + idx * 0.15, duration: 1, ease: [0.22, 1, 0.36, 1] }}
                    className={`h-full rounded-full ${bar.accent ? "bg-accent" : "bg-secondary/20"}`} />
                </div>
            ...
            <div className="text-center pt-2">
              <motion.span initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: 0.6 }} className="text-sm font-data font-bold text-accent">{data.benchmarks.speedup.toFixed(2)}× speedup</motion.span>
            </div>
          </div>
        </div>
      </Reveal>
...
function ConfusionGrid({ matrix, classes }: { matrix: number[][]; classes: string[] }) {
  ...
  if (!matrix.length) return <div className="py-10 text-xs font-data text-secondary text-center">No data.</div>;
