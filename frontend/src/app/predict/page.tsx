...
export default function PredictPage() {
  ...
  // Use env var for production (Vercel), fallback to localhost for dev
  ...
  return (
    <div className="space-y-7">
      ...
      <AnimatePresence>
        {showWarning && (
          <motion.div
            ...
            className="flex items-start gap-3 p-4 bg-amber-bg/40 border border-amber/25 rounded-2xl"
          >
            <AlertTriangle className="w-4 h-4 text-amber flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-foreground font-data">Report Extraction Complete</p>
              <p className="text-xs text-secondary mt-0.5">Highlighted fields were extracted from the file — review before running prediction.</p>
            </div>
            <button onClick={() => setShowWarning(false)} className="text-secondary hover:text-foreground transition-colors">
              <X className="w-4 h-4" />
            </button>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-7">
        ...
        >
          {/* Presets */}
          <div className="border border-border bg-card p-4 rounded-2xl space-y-2.5">
            <span className="text-[10px] uppercase tracking-[0.14em] font-bold text-secondary font-data block">Quick Presets</span>
            <div className="flex flex-col gap-1.5">
              {Object.entries(PRESETS).map(([name, cfg]) => (
                <motion.button
                  ...
                  className={`w-full text-left px-3 py-2.5 border text-xs font-semibold rounded-xl flex items-center gap-2 cursor-pointer transition-all duration-200 ${
                    activePreset === name
                      ? "border-accent bg-accent-tint/25 text-accent"
                      : "border-border bg-white hover:border-accent/30 text-foreground hover:bg-accent-tint/10"
                  }`}
                >
                  <span className="text-sm">{cfg.emoji}</span>
                  <span className="flex-1">{name}</span>
                  {activePreset === name && <ArrowRight className="w-3 h-3 text-accent" />}
                </motion.button>
              ))}
            </div>
          </div>
          ...
          {/* Upload */}
          <div className="border border-border bg-card p-4 rounded-2xl space-y-2.5">
            <span className="text-[10px] uppercase tracking-[0.14em] font-bold text-secondary font-data block">Upload Report</span>
            <motion.div
              onClick={() => !uploading && fileRef.current?.click()}
              whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
              className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-colors duration-300 ${
                uploading ? "border-accent/40 bg-accent-tint/10" : "border-border bg-white hover:border-accent/30 hover:bg-accent-tint/5"
              }`}
            >
              {uploading ? (
            ...
            <div className="flex items-start gap-1.5 p-2.5 bg-amber-bg/30 border border-amber/20 rounded-xl">
              <Info className="w-3 h-3 text-amber flex-shrink-0 mt-0.5" />
              <p className="text-[9px] text-secondary leading-relaxed">
                Extracted values are pre-filled. Always verify before computing.
              </p>
            </div>
          </div>
        </motion.div>
          ...
          <div className="border border-border bg-white p-6 md:p-8 rounded-2xl space-y-7">
            ...
            {GROUPS.map(g => (
              <div key={g.label} className="space-y-3">
                <h3 className="text-xs font-bold text-secondary border-b border-border pb-2">{g.label}</h3>
                    ...
                    <div key={f.key} className="space-y-1 group">
                      <div className="flex justify-between items-center min-h-[1.25rem]">
                        ...
                        {extracted[f.key] && (
                          <motion.span
                            ...
                            className="bg-amber-bg text-amber border border-amber/20 text-[8px] font-bold font-data px-1.5 py-0.5 rounded-md uppercase"
                          >
                            Verify
                          </motion.span>
                        )}
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-2 border rounded-xl text-sm font-data transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent ${
                          extracted[f.key]
                            ? "border-amber/40 bg-amber-bg/15 font-bold"
                            : "border-border bg-[#FFFDF8] hover:border-accent/25"
                        }`}
                      />
                    </div>
                  ))}
            ...
            <div className="flex justify-end pt-1">
              <motion.button
                onClick={predict}
                disabled={predicting}
                whileHover={{ y: -2, boxShadow: "0 8px 30px rgba(255,111,137,0.35)" }}
                whileTap={{ scale: 0.97 }}
                transition={{ type: "spring", stiffness: 400, damping: 20 }}
                className="px-7 py-3 bg-accent hover:bg-accent-hover disabled:bg-accent/60 text-white text-xs uppercase tracking-[0.15em] font-bold rounded-xl cursor-pointer shadow-[0_4px_16px_rgba(255,111,137,0.25)] transition-colors duration-200"
              >
                {predicting
                  ? <span className="flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Computing…</span>
                  : <span className="flex items-center gap-2"><FlaskConical className="w-3.5 h-3.5" /> Compute Diagnostic Likelihood</span>
                }
              </motion.button>
            </div>
          </div>
            ...
            {predicting && (
              ...
              >
                <div className="p-6 space-y-3">
                  <p className="text-[10px] font-bold text-secondary font-data uppercase tracking-[0.12em] mb-4">Quantum Pipeline Running…</p>
                  {STEPS.map((s, i) => (
                    <motion.div
                      key={i}
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="flex items-center gap-3 border border-accent/25 bg-accent-tint/15 p-4 rounded-2xl"
              >
                <AlertTriangle className="w-4 h-4 text-accent flex-shrink-0" />
                <p className="text-xs font-semibold text-accent flex-1">{error}</p>
                <button onClick={() => setError(null)} className="text-accent/60 hover:text-accent transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </motion.div>
            )}
                ...
                {results.quantum_kernel_svm && (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex-1">
                        <span className="text-[9px] text-secondary font-data uppercase tracking-wider block mb-1">
                          Quantum Kernel SVM · Multi-class One-vs-Rest
                        </span>
                        <motion.h2
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <div className="bg-white border border-border px-5 py-3 rounded-xl text-center">
                          <span className="text-[9px] text-secondary font-data block uppercase tracking-wider mb-0.5">Confidence</span>
                        ...
                        <div className="bg-sage-bg/50 border border-sage/20 px-4 py-3 rounded-xl text-center">
                          <span className="text-[9px] text-sage font-data block uppercase tracking-wider mb-0.5">Cohort</span>
                          ...
                          .map(([cls, val], idx) => {
                            ...
                            return (
                              <div key={cls} className="flex items-center gap-3">
                                <span className={`text-[11px] w-[125px] truncate flex-shrink-0 ${isPrimary ? "font-bold text-foreground" : "text-secondary"}`}>{cls}</span>
                                <div className="flex-1 h-2 bg-border/25 rounded-full overflow-hidden">
                                  <motion.div
                                    initial={{ width: 0 }}
                                    animate={{ width: `${Math.max(2, val * 100)}%` }}
                                    transition={{ delay: 0.25 + idx * 0.07, duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
                                    className={`h-full rounded-full ${isPrimary ? "bg-accent" : "bg-border"}`}
                                  />
                                </div>
                                <span className={`text-[11px] font-data font-bold w-11 text-right ${isPrimary ? "text-accent" : "text-secondary"}`}>
                                  {(val * 100).toFixed(1)}%
                                </span>
                              </div>
                            );
                    ...
                    <div className="space-y-2.5">
                      {PROXY_IMP.map(([feat, val], idx) => (
                        <div key={feat} className="flex items-center gap-3">
                          <span className="text-[11px] w-[105px] truncate flex-shrink-0 text-secondary font-semibold">{feat}</span>
                          <div className="flex-1 h-2 bg-border/25 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${(val / 0.185) * 100}%` }}
                              transition={{ delay: 0.35 + idx * 0.07, duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
                              className="h-full bg-secondary/30 rounded-full"
                            />
                          </div>
                          <span className="text-[11px] font-data font-bold text-secondary w-11 text-right">{val.toFixed(3)}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {([
                      ...
                      <motion.div
                        ...
                        transition={{ delay: 0.38 + idx * 0.08, type: "spring", stiffness: 300, damping: 24 }}
                        whileHover={{ y: -2 }}
                        className={`border p-5 rounded-xl text-center cursor-default transition-colors duration-200 ${
                          m.primary ? "border-accent/25 bg-accent-tint/10" : "border-border bg-card hover:border-accent/20"
                        }`}
                      >
                        <span className="text-[9px] text-secondary font-data uppercase tracking-wider block mb-2">{m.name}</span>
                        {m.obj ? (
                          <>
                            <div className="text-sm font-bold text-foreground">{m.obj.prediction}</div>
                            <div className={`text-sm font-data font-bold mt-0.5 ${m.primary ? "text-accent" : "text-secondary"}`}>
                              {(m.obj.confidence * 100).toFixed(1)}%
                            </div>
                          </>
                        ) : (
                          <div className="text-xs text-secondary italic font-data mt-2">Unavailable</div>
                        )}
                      </motion.div>
                    ))}
                  </div>
